...
4. **Name Mapping** (optional): Click the **Name Mapping...** button to anonymize speaker names in the exported dataset. The dialog lists all speaker names found across the scanned sessions. For each speaker, you can enter an alternative export name (e.g. `spk_01` instead of the real name). The mapped names are used for directory names, filenames, and all entries in `index.tsv`. Mappings are saved in preferences and restored automatically in future export sessions. Leave a field empty to keep the original name.
5. Click **Create Dataset** to create an organized dataset structure that's ready for TTS model training. The export process will handle file naming, metadata generation, and directory organization automatically.
